		...
		<div class = "bady">
			...
			@foreach ($vols as $s)
				@if ($s->number_of_vacansion > 0)
					@if ($s->activity_status == 1)
					
						<div class = "Left">
							<h2 id = "nameLeft">
								{{$s->activity_description}}
								
								<form action="/Edit_activity_{{$s->id}}">
									<button style = "margin-top: -2vh;"><img src = "/images/pencil.png" style = "width: 1.2vw;" alt = "symbol"></button>
								</form>
								<form action="#confirmation_activity_{{$s->id}}">
									<button style = "margin-top: 2vh;"><img src = "/images/bin.png" style = "width: 1.2vw;" alt = "symbol"></button>
								</form>
								<div id="confirmation_activity_{{$s->id}}" class="confirmation">
									<div class="okno">
										Вы точно хотите удалить мероприятие?
										<p>
											<a href="/result_deleting_activity_{{$s->id}}" class="button_comfirm">Да</a>
											<a href="./Navi2_edit" class="button_comfirm">Нет</a>
										</p>
									</div>
								</div>

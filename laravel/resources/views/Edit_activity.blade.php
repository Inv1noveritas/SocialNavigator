...
<html lang = "ru">
	...
	<body class = "Grid">
		...
		<div>
			<form action="/{{ $for_link }}_{{$id}}" method="post" name="my_form" id="form">
			@csrf
			<fieldset>
			<legend>Редактировать информацию о мероприятии</legend><br>			
				<div class = "form_grid">			
					<div class = "form_left">			
						<div class="field">
							<label for="description">Название мероприятия*:</label>
							<input type="text" id="description" name="activity_description" value="{{ $activity_description }}" autofocus required>
						</div>
						
						<div class="field">
							<label for="time">Дата и время проведения*:</label>
							<input type="datetime" id="time" name="activity_time" value="{{ $activity_time }}" required>
						</div>				

						<div class="field">
							<label for="location">Место проведения*:</label>
							<input type="text" id="location" name="activity_location" value="{{ $activity_location }}" required>
						</div>						
					</div>
					...
					<div class = "form_right">			
						<div class="field">
							...
							<input type="text" id="help" name="necessary_help" value="{{ $necessary_help }}" required>
						</div>
			
						<div class="field">
							<label for="number">Количество мест*:</label>
							<input type="number" id="number" name="number_of_vacansion" value="{{ $number_of_vacansion }}" value="" min="0" max="100" step="1" required>
						</div>
					</div>	
				</div>
